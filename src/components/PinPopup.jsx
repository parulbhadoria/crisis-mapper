...
export default function PinPopup({ pin, pins, onResolve, onUpvote, onConfirm, upvoted, }) {
  ...
  return (
    <div className="pin-popup min-w-[220px] max-w-[280px]">
      ...
      <div className="flex items-center gap-2 mb-2">
        <span
          className={`text-xs px-2 py-0.5 rounded-md font-semibold ${pin.severity === 'Critical'
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className={`font-semibold ${(pin.verificationScore || 0) >= 70
              ? 'text-green-600'
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-800">
                    {helper.category === "Medical" && "🚑 Medical Team"}
                    {helper.category === "Food" && "🍱 Food Supply"}
                    {helper.category === "Water" && "💧 Water Supply"}
                    {helper.category === "Shelter" && "🏠 Shelter"}
                    {helper.category === "Power Outage" && "⚡ Utility Team"}
                    {!helper.category && "🤝 Volunteer"}
                  </span>
